...
def is_known():
    ...
    data = pd.DataFrame(to_learn_dict)
...
unknown_button.config(highlightthickness=0, highlightbackground=BACKGROUND_COLOR)
unknown_button.grid(row=1,column=0)

check_image = PhotoImage(file="images/right.png")
known_button = Button(image=check_image, command=is_known)
known_button.config(highlightthickness=0, highlightbackground=BACKGROUND_COLOR)
